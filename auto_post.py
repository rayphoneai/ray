...
「はい」「承知」などの前置き・承諾文は一切出力禁止。本文のみ出力すること。"""
    ...
    # ── STEP 5b: X投稿（noteのURLを投稿→記事ごとのOGカード） ────
    if X_AUTO and X_COOKIES_B64:
        log("Xに投稿中...")
        try:
            # noteのURLがあればそちらを優先（OGタグが記事ごとに正しく設定される）
            x_post_url = note_result.get('url', '') if note_result.get('ok') else ''
            # noteのpublish URLを通常URLに変換
            if x_post_url and 'editor.note.com' in x_post_url:
                # https://editor.note.com/notes/nXXXXX/publish/ → https://note.com/rayphone/n/nXXXXX
                import re as _re
                m = _re.search(r'/notes/(n[a-f0-9]+)/', x_post_url)
                if m:
                    cfg_note = os.getenv('NOTE_URL', 'https://note.com/rayphone').rstrip('/')
                    urlname = cfg_note.split('/')[-1]
                    x_post_url = f"https://note.com/{urlname}/n/{m.group(1)}"
            if not x_post_url:
                x_post_url = art_url  # fallback to blog URL
            log(f"X投稿予定URL: {x_post_url}")
