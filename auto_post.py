...
Rayphoneの一人称・体験談必須。# * 【】絶対禁止。合計3000字前後。""", 4000))
    ...
    # 既存記事を取得（raw.githubusercontent.com経由 → 1MB超えも対応）
    arts = []
    for attempt in range(3):
        try:
            raw_url = (f"https://raw.githubusercontent.com/{GH_USER}/{GH_REPO}"
                       f"/main/articles.json?t={int(time.time())}")
