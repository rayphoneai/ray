...
「はい」「承知」などの前置き・承諾文は一切出力禁止。本文のみ出力すること。"""
    ...
    # note投稿（APIを優先、失敗したらPlaywright）
    log("noteに投稿中...")
    note_result = {"ok": False, "message": "未試行"}
    try:
        note_result = post_to_note_api(f"【深掘り】{meta['title']}", note_content, art_url)
    except Exception as e:
        log(f"note API失敗: {e} → Playwright方式を試みます")
        try:
            import importlib.util, sys as _sys
            _spec = importlib.util.spec_from_file_location("note_poster", Path(__file__).parent / "note_poster.py")
            _mod = importlib.util.module_from_spec(_spec)
            _spec.loader.exec_module(_mod)
            note_result = _mod.post_to_note(f"【深掘り】{meta['title']}", note_content, svg_code, publish=True, blog_url=art_url)
        except Exception as e2:
            log(f"✗ note投稿エラー: {e2}")
            note_result = {"ok": False, "message": str(e2)}
