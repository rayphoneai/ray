...
# ── カテゴリインデックス管理 ─────────────────────────────
def get_cat_index():
    # GitHub上のcat_index.txtから読み込み（Actions間で永続化）
    if GH_TOKEN and GH_USER and GH_REPO:
        try:
            ...
            if r.status_code == 200:
                val = base64.b64decode(r.json()["content"]).decode().strip()
                return int(val)
        except Exception:
            pass
    # ローカルファイルをフォールバック
...
def save_cat_index(idx):
    ...
    # GitHub上のcat_index.txtも更新（Actions間で永続化）
    if GH_TOKEN and GH_USER and GH_REPO:
        try:
            url = f"https://api.github.com/repos/{GH_USER}/{GH_REPO}/contents/cat_index.txt"
            headers = {"Authorization": f"token {GH_TOKEN}", "Accept": "application/vnd.github.v3+json",
            ...
            # PUT
            body = {
                "message": f"Update cat_index to {idx}",
                "content": base64.b64encode(str(idx).encode()).decode()
            }
            if sha:
                body["sha"] = sha
...
def main():
    ...
    _now = datetime.now()
    _seed_words = [
        "初心者でも即実践できる","上級者向け深掘り","意外と知らない","失敗から学んだ",
        "現場で本当に使える","5分でわかる","収益につながる","時短を極める",
        "プロが教える","実例で解説する","2025年最新版","よくある間違いと対策",
        "具体的な手順","比較検証","活用アイデア10選"
    ]
    _angles = [
        "初心者向けの入門解説","実践者向けの応用テクニック","業務効率化の観点",
        "副業・収益化の観点","時短・自動化の観点","プロンプト設計の観点",
        "失敗事例と改善策","ビフォーアフター事例","ステップバイステップ解説"
    ]
    _rand_word  = _random.choice(_seed_words)
    _rand_angle = _random.choice(_angles)
    _date_hint  = _now.strftime("%Y年%m月%d日 %H:%M")
    _rand_num   = _random.randint(1000, 9999)

    plan_prompt = f"""あなたはRayphone（プロンプト設計士・商品開発15年・Claude副業月収15万）のブログ「RayPhoneAI」の記事企画担当です。
カテゴリ「{cat}」で、AIを活用した実践的な記事企画を1件作成してください。

【生成条件】
- 生成日時: {_date_hint}（毎回ユニークな企画を作ること）
- 切り口のヒント: 「{_rand_word}」×「{_rand_angle}」
- シード: {_rand_num}
- 過去と重複しないよう、具体的で独自性のあるタイトルにすること

JSON形式で出力:
{{"title":"記事タイトル（30字以内・具体的で独自性があること）","target":"ターゲット読者","keywords":"SEOキーワード","hook":"読者の悩みや課題"}}
...
「はい」「承知」などの前置き・承諾文は一切出力禁止。本文のみ出力すること。"""
    ...
    # ── STEP 5b: X投稿（noteのURLを直接タイピングで投稿） ────
    if X_AUTO and X_COOKIES_B64:
        log("Xに投稿中...")
        try:
            # note投稿成功時はnote記事URL、失敗時はblog URLをフォールバック
            x_post_url = note_result.get('url', '') if note_result.get('ok') else ''
            if not x_post_url:
                x_post_url = art_url
            log(f"X投稿URL: {x_post_url}")
            post_to_x_actions(x_post_url)
        except Exception as e:
            log(f"✗ X投稿エラー: {e}")
...
def post_to_x_actions(art_url: str):
    """GitHub Actions環境でX投稿（URLのみ・カード表示）"""
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            # ボタン有効化待機（URLだけなら即有効化される）
            time.sleep(2)
            for wait_i in range(10):
                try:
                    is_dis = page.evaluate("() => { const b = document.querySelector('[data-testid=\"tweetButton\"]'); return b ? (b.disabled || b.getAttribute('aria-disabled') === 'true') : true; }")
                    if not is_dis:
                        log(f"X: ボタン有効化 ({wait_i+1}秒)")
                        break
                except Exception:
                    pass
                time.sleep(1)
...
"""
            ...
            # editor.note.com/notes/nXXXXX/publish/ → note.com/rayphone/n/nXXXXX に変換
            import re as _re
            m = _re.search(r'/notes/(n[a-f0-9]+)/', note_url)
            if m:
                urlname = NOTE_URL.rstrip('/').split('/')[-1] if NOTE_URL else 'rayphone'
                note_url = f"https://note.com/{urlname}/n/{m.group(1)}"
                log(f"note: 記事URL={note_url}")
