...
Rayphoneの一人称・体験談必須。# * 【】絶対禁止。合計3000字前後。""", 4000))
    ...
    for attempt in range(3):
        try:
            ...
            log(f"GitHub API ステータス: {r.status_code}")
            if r.status_code == 200:
                resp_json = r.json()
                # encoding フィールドを確認
                encoding = resp_json.get("encoding", "base64")
                log(f"encoding: {encoding}, content長: {len(resp_json.get('content',''))}文字")
                if encoding == "base64":
                    raw_b64 = resp_json["content"].replace("\n", "").replace("\r", "")
                    decoded = base64.b64decode(raw_b64).decode("utf-8")
                else:
                    decoded = resp_json["content"]
                log(f"デコード後: {len(decoded)}文字, 先頭: {repr(decoded[:50])}")
                arts = json.loads(decoded)
                fetch_ok = True
                ...
                log(f"取得失敗 HTTP {r.status_code}: {r.text[:100]}")
        except Exception as e:
            log(f"既存記事取得失敗(試行{attempt+1}): {type(e).__name__}: {e}")
            time.sleep(3)

    if not fetch_ok:
        log("⚠ 既存記事の取得に3回失敗。新規作成として続行します。")
        arts = []  # 安全策：空リストで続行（中断しない）
